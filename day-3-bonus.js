...
let firstNames  = people.reduce((acc, person, index) => {
...
let lastNames = employees.reduce((acc, person, index) => {
  return acc + person.lastName + (index < employees.length - 1 ? ", " : "");
}, "");
...
let  productNames2 = products.reduce((acc, product, index) => {
  return acc + product.productName + (index < products.length - 1 ? ", " : "");
}, "");
console.log("Product Names:", productNames2);
...
let taskNameList = tasks.reduce((acc, task, index) => {
  return acc + task.description + (index < tasks.length - 1 ? ", " : "");
}, "");
console.log("Task Names:", taskNameList);
...
let animalSounds = exampleAnimals.reduce((acc, sound, index) => {
  return acc + sound.sound + (index < exampleAnimals.length - 1 ? ", " : "");
}, "");
