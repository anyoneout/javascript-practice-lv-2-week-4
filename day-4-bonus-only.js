...
let myProducts = [];
myProducts.push(createProduct("Brocolli Phone", 500));
myProducts.push(createProduct("Brocolli Skates", 1500));
myProducts.push(createProduct("Brocolli Dishwasher", 1400));

let discountedMYProducts = myProducts.map(product => product.applyDiscount(0.02));
console.log("Discounted Products:", discountedMYProducts);
...
let myStudents = [];
myStudents.push(createStudent("Manny", 15));
myStudents.push(createStudent("Moe", 17));
myStudents.push(createStudent("Jack", 19));

let myStudentsImproved = myStudents.map(student => student.improveGrade(10));
console.log("Improved Students:", myStudentsImproved);

// EXERCISE 3: Formatting Product Nam
// INSTRUCTIONS: Create an empty array called `products`.
// Use the `push` method to add 3 products to the array using the `createProduct` factory function.
...
let myProducts2 = [];
myProducts2.push(createProduct("Brocolli Jumpsuit", 250));
myProducts2.push(createProduct("Brocolli Computer", 1200));
myProducts2.push(createProduct("Brocolli Headphones", 120));

let myProducts2UpperCase = myProducts2.map(product => product.name.toUpperCase());
console.log("Product Names in Uppercase:", myProducts2UpperCase);
...
let myStudents2 = [];
myStudents2.push(createStudent("Yakko", 75));
myStudents2.push(createStudent("Dot", 95));
myStudents2.push(createStudent("Wakko", 55));

let gradeAdd = myStudents2.map(student => {
  student.grade += 2;
  return student.grade;
});
console.log("Changed Grades:", gradeAdd);
...
let myTasks2 = [];
myTasks2.push({ description: "Make a table", dueDate: "2024-08-29" });
myTasks2.push({ description: "Make a chair", dueDate: "2024-08-30" });
myTasks2.push({ description: "Sit at table", dueDate: "2024-09-01" });

let uppercaseTasks2 = myTasks2.map(task => task.description.toUpperCase());
console.log("Uppercase Task Descriptions", uppercaseTasks2);
